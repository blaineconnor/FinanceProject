...
class UserMainVC: UIViewController, UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource, CustomAlertDelegate{
    ...
    override func viewDidLoad() {
        ...
        _ = vm.cur.subscribe(onNext: { [weak self] liste in
            self?.cur = liste
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let collectionCell = collectionView.dequeueReusableCell(withReuseIdentifier: "UserCollectionViewCell", for: indexPath) as! UserCollectionViewCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let selectedCurrency = cur[indexPath.item]
        navigateToAlimSatimVC(with: selectedCurrency)
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let selectedKur = cur[indexPath.row]
        navigateToAlimSatimVC(with: selectedKur)
    }
...
extension UserMainVC: UICollectionViewDelegateFlowLayout {
    ...
    func navigateToAlimSatimVC(with selectedKur: Kurlar) {
        let alimSatimVC = storyboard?.instantiateViewController(withIdentifier: "AlimSatimVC") as! AlimSatimVC
        alimSatimVC.selectedKur = selectedKur
        alimSatimVC.delegate = self // Eğer gerekliyse delegate'yi ayarlayın
        present(alimSatimVC, animated: true, completion: nil)
    }
